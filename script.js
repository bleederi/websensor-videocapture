...
function buildCameraPath(dataArray) {    //Build the shaky camera path from the sensor measurements (convert to canvas coordinates) using projection
        for (let i=0; i<dataArray.length; i++)
        {
                ...
                if(ori !== undefined)
                {
                        ...
                        cameraCoord.x = canvas.width/2 + videoElement.videoWidth * Math.sin(oriDiff.yaw);
                        cameraCoord.y = canvas.height/2 + videoElement.videoHeight * Math.sin(oriDiff.alpha);
